...
poses = [cobra, seal, upDog, downDog, child, extChild, hero, heroToes, staff, headKnee, mountain, volcano, tree,
         ...
         lizardLunge_L, highLunge_R, highLunge_L, sideLunge_R, sideLunge_L, yogi, frog, chair, standPigeon_R, standPigeon_L]


#Sequence parameters: style, primary body parts to focus, duration and intensity will be chosen by the user
#Primaries, duration and intensity code is in progress
styleInput = input("What style of yoga would you like to practise?\n")
style = styleInput[0:3].lower()
primariesInput = ["legs", "arms", "back", "shoulders", "hips"]
primaries = primariesInput
duration = 600
warmUp = duration * 0.2
...
#addPose function selects a pose that fits the chosen body part focus and adds it to the sequence
...
#checkSymmetry checks to see if a pose has a corresponding left or right pose and then immediately adds it to the sequence
...
#pickWarmUp is used whilst the sequence length is in the warm-up phase to select warm-up poses
...
#pickYin is used whilst the sequence length is in the main flow phase to select yin poses
def pickYin(primaries):
    length = 0
    while length < mainFlow:
        potential = random.choice(poses)
        if potential.is_yin:
            length = addPose(primaries, potential, length) + 10
            length = checkSymmetry(sequence, length) + 10

#pickVin is used whilst the sequence length is in the main flow phase to select vinyasa poses
...
#pickPow is used whilst the sequence length is in the main flow phase to select power yoga poses
...
#pickCoolDown is used whilst the sequence length is in the cool-down phase to select cool-down poses
...
#This code is dependent on the style of yoga chosen by the user. It compiles the sequence and prints it for the user to read.
if style == "yin":
